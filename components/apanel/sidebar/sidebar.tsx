            ...
            <div className="p-4">
                <ul>
                    ...
                    {dropdownRoutes.map((route: dropdownRoute, index: number) => (
                        <div key={index}>
                            <li
                                className={`flex items-center p-3 ${selected == route.name.toLowerCase() ? "bg-gray-700" : ""} cursor-pointer`}
                                ...
                                <ul className={`ml-4`}>
                                    {route.subRoutes.map((subRoute: route, index: number) => (
                                        <Link href={subRoute.path} key={index} onClick={() => setSelected(subRoute.name.toLowerCase())}>
                                            <li className={`flex items-center p-3 ${selected == subRoute.name.toLowerCase() ? "bg-gray-700" : ""}`}>
                                                {subRoute.icon}
